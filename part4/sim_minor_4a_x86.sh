...
for opLat in {1..6}; do
    ...
    ${GEM5_BIN} --outdir=${OUTDIR} config_minor_4a_x86.py ${opLat} ${issueLat}
    
    echo "opLat=${opLat} issueLat=${issueLat}" >> m5out/all_results_4a_x86.txt
    cat ${OUTDIR}/stats.txt | grep -E "board.processor.cores.core\.(cpi|numCycles)|commitStats0.numInsts|simSeconds" >> m5out/all_results_4a_x86.txt
    echo -e "\n\n" >> m5out/all_results_4a_x86.txt
done