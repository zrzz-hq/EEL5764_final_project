...
# Clear results file
> m5out/all_results_4b_riscv.txt
...
run_test() {
    ...
    ${GEM5_BIN} --outdir=${OUTDIR} config_minor_4b_riscv.py ${int_opLat} ${float_opLat}
    
    echo "int_opLat=${int_opLat} float_opLat=${float_opLat}" >> m5out/all_results_4b_riscv.txt
    cat ${OUTDIR}/stats.txt | grep -E "board.processor.cores.core\.(cpi|numCycles)|commitStats0.numInsts|simSeconds" >> m5out/all_results_4b_riscv.txt
    echo -e "\n\n" >> m5out/all_results_4b_riscv.txt
}
